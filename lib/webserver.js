...
// -
        var meta_manager = new meta_factory(working_dir, user_config.meta_dir)
        var optimizer = new optimizer_factory(meta_manager, user_config)
                ...
                if( build_enabled && is_build_required(req) ){
                    ...
                    if( assets_injection_enabled ){
                        ...
                        if( user_config.scripts ){
                            optimizer.create_combined_assets(user_config.scripts, user_config.build_run_paths);
                            buf = phantomizer_helper.apply_scripts(user_config.scripts, base_url, buf);
                        }
                        if( user_config.css ){
                            optimizer.create_combined_assets(user_config.css, user_config.build_run_paths);
                    ...
                    if( build_enabled
                        ...
                        if( optimizer.is_build_fresh(buidl_profile(req), request_path) ){
                            ...
                            if( assets_injection_enabled && optimizer.is_combined_asset(user_config.scripts, request_path) ){
                                optimizer.create_combined_asset(user_config.scripts, user_config.build_run_paths, request_path, function(){
                                    var file = file_utils.find_file([user_config.out_dir], request_path);
                                    var buf = fs.readFileSync(file);
                                    respond(200, headers, buf, res);
                                });
                            }else if( assets_injection_enabled && optimizer.is_combined_asset(user_config.css, request_path) ){
                                optimizer.create_combined_asset(user_config.css, user_config.build_run_paths, request_path, function(){
                                    var file = file_utils.find_file([user_config.out_dir], request_path);
                                    var buf = fs.readFileSync(file);
                                    respond(200, headers, buf, res);
                                });
...
// -
        ...
        function inject_extras(req,buf){
            ...
            if( inject ){
                ...
                var ncontent = "";
                ncontent += "<script >"
                ncontent += "require(['vendors/go-phantomizer/extras-loader']);"
                ncontent += "</script>"
