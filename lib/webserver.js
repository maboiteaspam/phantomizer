                ...
                if( build_enabled && is_build_required(req) ){
                    ...
                    if( assets_injection_enabled ){
                        ...
                        if( user_config.scripts ){
                            create_combined_assets(user_config.scripts, user_config.build_run_paths);
                            buf = phantomizer_helper.apply_scripts(user_config.scripts, base_url, buf);
                        }
                        if( user_config.css ){
                            create_combined_assets(user_config.css, user_config.build_run_paths);
...
// -
        ...
        function inject_extras(req, request_path, buf){
            if( dashboard_enabled && is_dashboard_friend(req, request_path, buf) ){
                buf = phantomizer_helper.inject_dashboard(requirejs_baseUrl, requirejs_src, buf)
            }
            if( is_previewing_device(req, request_path, buf) ){
                buf = phantomizer_helper.inject_device_preview(requirejs_baseUrl, requirejs_src,  buf, req.query.device,req.query.device_mode)
            }
            if( test_required(req) ){
                if( is_phantomizing ){
                    buf = phantomizer_helper.inject_phantom( buf);
                }
                buf = phantomizer_helper.inject_qunit(requirejs_baseUrl, requirejs_src, buf);
            }
            return buf;
        };
